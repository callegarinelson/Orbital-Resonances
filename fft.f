     ...
     |xpla(7,6),f(7),bx(7,1),by(7,1),bz(7,1)
      REAL*8 XRES1FFT(1048576),XRES2FFT(1048576),XW2FFT(1048576)
      REAL*8 XA2FFT(1048576),XE2FFT(1048576)
...
199	Format(F14.8,1x,F14.8,1x,I5)
      ...
      aa=1.680332379507223E+05 /60268.0d0
      exc=3.212287757274269E-03
...
c	fatorinf=197900.0d0
c	fatorsup=198400.0d0
c	do fator=fatorinf,fatorsup,(fatorsup-fatorinf)/700.0d0

c	aa=fator/60268.0d0
	ee=exc
...
cccccccccccccccccccccccccccccc  FIM   Definicoes: 
      CALL XYZORB(N,xpla,a,e,di,w,om,am,ami,lei,kk) 
     	...
     	dlamb1=am(1)+w(1)+om(1)
	dlamb2=am(2)+w(2)+om(2)
	aux=7.d0*dlamb2-6.d0*dlamb1
	dp1=w(1)+om(1)
	dp2=w(2)+om(2)
	res1= aux-dp1
	res2= aux-dp2

      res1=res1-aint(res1/pi2)*pi2
      if (res1.lt.0.0d0) res1=res1+pi2

      res2=res2-aint(res2/pi2)*pi2
      if (res2.lt.0.0d0) res2=res2+pi2

      WW2=w(2)-aint(w(2)/pi2)*pi2
      if (WW2.lt.0.0d0) WW2=WW2+pi2
...
c	pause
      ...
      IJK=89
      CALL  FFT(ijk,Xres1FFT,dk1)

      IJK=899
      CALL  FFT(ijk,Xres2FFT,dk1)
...
C     >XI3FFT,XA4FFT,XA5FFT,XE5FFT,XA6FFT)
	...
	INTEGER nespec01,nespec05,nespec1,nespec3,
     >nespec5,nespec10,nespec15,dn01,dn05,dn1,dn3,dn5,dn10,dn15
      ...
      open (789  ,file='especres1.DAT', status='unknown')
      open (7899 ,file='especres2.DAT', status='unknown')
      open (78999,file='especA.DAT', status='unknown')
      open (78199,file='especE.DAT', status='unknown')
      open (78139,file='especI.DAT', status='unknown')
...
C      if(n.eq.0.and.ijk.eq.8999) write(24,*) gg(n+1),tf/2.0d0
	...
	FRE(ii)=gg(n+1)
	AMP(ii)=tf
	if(ijk.eq.89)  write(789, *)fre(ii),1.0D0/fre(ii),amp(ii)
	if(ijk.eq.899) write(7899,*)fre(ii),1.0D0/fre(ii),amp(ii)
	if(ijk.eq.8999)write(78999,*)fre(ii),1.0D0/fre(ii),amp(ii)
	if(ijk.eq.8199)write(78199,*)fre(ii),1.0D0/fre(ii),amp(ii)
	if(ijk.eq.8139)write(78139,*)fre(ii),1.0D0/fre(ii),amp(ii)
