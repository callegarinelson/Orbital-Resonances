     ...
     |xpla(7,6),f(7),bx(7,1),by(7,1),bz(7,1)
      REAL*8 XRES1FFT(2097152),XRES2FFT(2097152),XW2FFT(2097152)
      REAL*8 XA2FFT(2097152),XE2FFT(2097152)

	
c     >XE2FFT(2097152),XA3FFT(2097152),XI3FFT(2097152),
c     >XA4FFT(2097152),XA5FFT(2097152),XE5FFT(2097152),XA6FFT(2097152)
      REAL*8 NLINHAS,DELTAT,ttf,tint
	INTEGER nespec01,nespec05,nespec1,
     >nespec5,nespec10,nespec15,dn01,dn05,dn1,dn5,dn10,dn15,ijk
...
c	write(*,*)ttp,tint,tint/365.25d0

c     km
c      ua=1.49597870691d8
c       sss=1.324243E-03*ua
c       write(*,*)sss
...
c      read(*,*) N
	...
	fatorinf=167970.0d0
	fatorsup=168090.0d0
	do fator=fatorinf,fatorsup,(fatorsup-fatorinf)/1500.0d0
...
cc     	dlamb1=am(1)+w(1)
c	dlamb2=am(2)+w(2)
c	aux=15.d0*dlamb2-14.d0*dlamb1
c	dp1=w(1)+om(1)
c	dp2=w(2)+om(2)
c	res1= aux-dp1
c	res2= aux-dp2

c      res1=res1-aint(res1/pi2)*pi2
c      if (res1.lt.0.0d0) res1=res1+pi2

c      res2=res2-aint(res2/pi2)*pi2
c      if (res2.lt.0.0d0) res2=res2+pi2

c      WW2=w(2)-aint(w(2)/pi2)*pi2
c      if (WW2.lt.0.0d0) WW2=WW2+pi2

c      Xres1FFT(1)=a(2)*dcos(res1)
c      Xres2FFT(1)=e(2)*dcos(res1)
      Xw2FFT(1)=di(2)
      Xa2FFT(1)=a(2)
      Xe2FFT(1)=e(2)
...
c     	dlamb1=am(1)+w(1)
c	dlamb2=am(2)+w(2)
c	aux=11.d0*dlamb2-10.d0*dlamb1
c	dp1=w(1)+om(1)
c	dp2=w(2)+om(2)
c	res1= aux-dp1
c	res2= aux-dp2
...
c      Xres1FFT(jjj)=a(2)*dcos(res1)
c      Xres2FFT(jjj)=e(2)*dcos(res1)
      Xw2FFT(jjj)=di(2)
      Xa2FFT(jjj)=a(2)
      Xe2FFT(jjj)=e(2)

c      write(*,*)jjj,Xe2FFT(jjj)
	jjj=jjj+1      
...
c      IJK=1
c      CALL FFT(ijk,XRES1FFT,dk1)

c      IJK=2
c      CALL FFT(ijk,XRES2FFT,dk1)

      IJK=3
      CALL  FFT(ijk,XW2FFT,dk1)

      IJK=4
      CALL FFT(ijk,XA2FFT,dk1)

      IJK=5
      CALL  FFT(ijk,XE2FFT,dk1)
...
cccccccccccccccccccccccccccccccccccccccccccccccccc
      SUBROUTINE FFT(ijk,XE2FFT,dk1)
c      SUBROUTINE FFT(ijk,XE2FFT,nespec01,nespec05,nespec1,nespec3,
c     >nespec5,nespec10,nespec15,dk1)
C	(XA1FFT,XI1FFT,XA2FFT,XE2FFT,XA3FFT,
C     >XI3FFT,XA4FFT,XA5FFT,XE5FFT,XA6FFT)
      INTEGER n,nn,i,ii,j,kk,isign,ijk
      REAL*8 data(4194304)
      REAL*8 XE2FFT(2097152)
      REAL*8 FRE(2097152),AMP(2097152),FFRE(2097152)
      REAL*8 AAMP(2097152),ff(2097152),gg(2097152)
      REAL*8 NLINHAS,DELTAT,ttf,tint,dk1
      REAL*8 tf
...
c      open (2269,file='especA.DAT', status='unknown')
c      open (23,file='especE.DAT', status='unknown')
c      open (24,file='especI.DAT', status='unknown')
c

      IF (IJK.EQ.1) THEN
      OPEN(1,FILE='edh8RES1.dat',STATUS='UNKNOWN')
      OPEN(10,FILE='edh10RES1.dat',STATUS='UNKNOWN')
      OPEN(100,FILE='edh13RES1.dat',STATUS='UNKNOWN')
      OPEN(1000,FILE='edh15RES1.dat',STATUS='UNKNOWN')
      OPEN(10000,FILE='edh18RES1.dat',STATUS='UNKNOWN')
      OPEN(100000,FILE='edh21RES1.dat',STATUS='UNKNOWN')
      OPEN(1000000,FILE='edh25RES1.dat',STATUS='UNKNOWN')
      OPEN(10000000,FILE='edh30RES1.dat',STATUS='UNKNOWN')
      OPEN(100000000,FILE='edh40RES1.dat',STATUS='UNKNOWN')

      ENDIF

      IF (IJK.EQ.2) THEN
      OPEN(2,FILE='edh8RES2.dat',STATUS='UNKNOWN')
      OPEN(20,FILE='edh10RES2.dat',STATUS='UNKNOWN')
      OPEN(200,FILE='edh13RES2.dat',STATUS='UNKNOWN')
      OPEN(2000,FILE='edh15RES2.dat',STATUS='UNKNOWN')
      OPEN(20000,FILE='edh18RES2.dat',STATUS='UNKNOWN')
      OPEN(200000,FILE='edh21RES2.dat',STATUS='UNKNOWN')
      OPEN(2000000,FILE='edh25RES2.dat',STATUS='UNKNOWN')
      OPEN(20000000,FILE='edh30RES2.dat',STATUS='UNKNOWN')
      OPEN(200000000,FILE='edh40RES2.dat',STATUS='UNKNOWN')

      ENDIF

      IF (IJK.EQ.3) THEN
      OPEN(3,FILE='edh1in.dat',STATUS='UNKNOWN')
      OPEN(30,FILE='edh5in.dat',STATUS='UNKNOWN')
      OPEN(300,FILE='edh10in.dat',STATUS='UNKNOWN')
      OPEN(3000,FILE='edh15in.dat',STATUS='UNKNOWN')
      OPEN(30000,FILE='edh20in.dat',STATUS='UNKNOWN')
      OPEN(300000,FILE='edh25in.dat',STATUS='UNKNOWN')
      OPEN(3000000,FILE='edh30in.dat',STATUS='UNKNOWN')
      OPEN(30000000,FILE='edh35in.dat',STATUS='UNKNOWN')
      OPEN(33000000,FILE='edh40in.dat',STATUS='UNKNOWN')

      ENDIF

      IF (IJK.EQ.4) THEN
      OPEN(4,FILE='edh1a2.dat',STATUS='UNKNOWN')
      OPEN(40,FILE='edh5a2.dat',STATUS='UNKNOWN')
      OPEN(400,FILE='edh10a2.dat',STATUS='UNKNOWN')
      OPEN(4000,FILE='edh15a2.dat',STATUS='UNKNOWN')
      OPEN(40000,FILE='edh20a2.dat',STATUS='UNKNOWN')
      OPEN(400000,FILE='edh25a2.dat',STATUS='UNKNOWN')
      OPEN(4000000,FILE='edh30a2.dat',STATUS='UNKNOWN')
      OPEN(40000000,FILE='edh35a2.dat',STATUS='UNKNOWN')
      OPEN(44000000,FILE='edh40a2.dat',STATUS='UNKNOWN')
      ENDIF

      IF (IJK.EQ.5) THEN
      OPEN(5,FILE='edh1e2.dat',STATUS='UNKNOWN')
      OPEN(50,FILE='edh5e2.dat',STATUS='UNKNOWN')
      OPEN(500,FILE='edh10e2.dat',STATUS='UNKNOWN')
      OPEN(5000,FILE='edh15e2.dat',STATUS='UNKNOWN')
      OPEN(50000,FILE='edh20e2.dat',STATUS='UNKNOWN')
      OPEN(500000,FILE='edh25e2.dat',STATUS='UNKNOWN')
      OPEN(5000000,FILE='edh30e2.dat',STATUS='UNKNOWN')
      OPEN(50000000,FILE='edh35e2.dat',STATUS='UNKNOWN')
      OPEN(55000000,FILE='edh40e2.dat',STATUS='UNKNOWN')
      ENDIF
...
c      if(n.eq.0.and.ijk.eq.89) write(2269,*) gg(n+1),tf/2.0d0
c      if(n.eq.0.and.ijk.eq.899) write(23,*) gg(n+1),tf/2.0d0
c      if(n.eq.0.and.ijk.eq.8999) write(24,*) gg(n+1),tf/2.0d0

	if(n.ge.1.and.n.le.nn/2-1) THEN
	FRE(ii)=gg(n+1)
	AMP(ii)=tf
c	if(ijk.eq.89)write(2269,*)fre(ii),1.0D0/fre(ii),amp(ii)
c	if(ijk.eq.899)write(23,*)fre(ii),1.0D0/fre(ii),amp(ii)
c	if(ijk.eq.8999)write(24,*)fre(ii),1.0D0/fre(ii),amp(ii)
	ENDIF
...
C	JJ e' o numero total de picos
	...
	if(ampl.gt. 1.d0*amplim ) then
	write(1,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 5.d0*amplim ) then
	write(10,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 10.0d0*amplim  ) then
	write(100,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 15.0d0*amplim  ) then
	write(1000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 20.0d0*amplim  ) then
	write(10000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 25.0d0*amplim ) then
	write(100000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 30.0d0*amplim  ) then
	write(1000000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 35.0d0*amplim  ) then
	write(10000000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 40.0d0*amplim  ) then
	write(100000000,*)dk1*60268.0d0,peri
	endif
	...
	if(ampl.gt. 1.d0*amplim ) then
	write(2,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 5.d0*amplim ) then
	write(20,*)dk1*60268.0d0,peri
	endif

	if( ampl.gt.10.d0*amplim  ) then
	write(200,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 15.0d0*amplim  ) then
	write(2000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 20.d0*amplim  ) then
	write(20000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 25.0d0*amplim ) then
	write(200000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 30.0d0*amplim  ) then
	write(2000000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 35.0d0*amplim  ) then
	write(20000000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 40.0d0*amplim  ) then
	write(200000000,*)dk1*60268.0d0,peri
	endif
	...
	if(ampl.gt. 1.d0*amplim ) then
	write(3,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt.5.d0*amplim ) then
	write(30,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 10.0d0*amplim  ) then
	write(300,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 15.d0*amplim  ) then
	write(3000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 20.d0*amplim  ) then
	write(30000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 25.0d0*amplim ) then
	write(300000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 30.0d0*amplim  ) then
	write(3000000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 35.0d0*amplim  ) then
	write(30000000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 40.0d0*amplim  ) then
	write(33000000,*)dk1*60268.0d0,peri
	endif
	...
	if(ampl.gt. 1.d0*amplim ) then
	write(4,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 5.d0*amplim ) then
	write(40,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 10.d0*amplim  ) then
	write(400,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 15.d0*amplim  ) then
	write(4000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 20.d0*amplim  ) then
	write(40000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 25.0d0*amplim ) then
	write(400000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 30.0d0*amplim  ) then
	write(4000000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 35.0d0*amplim  ) then
	write(40000000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 40.0d0*amplim  ) then
	write(44000000,*)dk1*60268.0d0,peri
	endif
      ENDIF

      IF (IJK.EQ.5) THEN
	if(ampl.gt. 1.d0*amplim ) then
	write(5,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 5.d0*amplim ) then
	write(50,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 10.d0*amplim  ) then
	write(500,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 15.d0*amplim  ) then
	write(5000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 20.d0*amplim  ) then
	write(50000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 25.0d0*amplim ) then
	write(500000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 30.0d0*amplim  ) then
	write(5000000,*)dk1*60268.0d0,peri
	endif

       	if(ampl.gt. 35.0d0*amplim  ) then
	write(50000000,*)dk1*60268.0d0,peri
	endif

	if(ampl.gt. 40.0d0*amplim  ) then
	write(55000000,*)dk1*60268.0d0,peri
	endif
